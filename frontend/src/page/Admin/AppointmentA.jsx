...
function AppointmentA() {
  ...
  const fetchAppointments = async () => {
    try { 
      const response = await axios.get(
        "http://localhost:8081/manager_appointments"
      );
  ...
  const filteredAppointments = appointments.filter(
    (appointment) =>
      appointment.Branch_Name.toLowerCase().includes(
  ...
  return (
     /*Show the supplier details are shown */
    ...
    <div>
      ...
      <div style={{ display: "flex" }}>
        ...
        <div>
          ...
          />
          <Card style={{ margin: "0 20px", minWidth: "700px" }}>
            {" "}
            {/* Add right margin and set min-width */}
            <CardContent>
              <Table>
                <TableHead>
                  <TableRow>
                ...
                <TableBody>
                  {filteredAppointments.map((appointment) => (
                    <TableRow key={appointment.Supplier_ID}>
                      ...
                      <TableCell>
                        {moment(appointment.Selected_Date).format("YYYY-MM-DD")}
                      </TableCell>
